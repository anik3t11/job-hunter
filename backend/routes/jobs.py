...
@router.get("/stats")
def job_stats(user: dict = Depends(get_current_user)):
    return get_stats(user["user_id"])


@router.get("/export/csv")
def export_csv(user: dict = Depends(get_current_user)):
    csv_data = export_jobs_csv(user["user_id"])
    return Response(
...
@router.patch("/{job_id}")
def patch_job(job_id: int, body: StatusUpdate, user: dict = Depends(get_current_user)):
    job = get_job_detail(job_id, user["user_id"])
    if not job:
        raise HTTPException(404, "Job not found")
    ...
    if body.status is not None:
        if body.status not in {"new", "reviewed", "applied", "interview", "rejected", "offer"}:
    ...
    if updates:
        update_job(job_id, user["user_id"], updates)
    return {"ok": True}


@router.delete("/clear/all")
def delete_all_jobs(user: dict = Depends(get_current_user)):
    clear_jobs(user["user_id"])
    return {"ok": True}
